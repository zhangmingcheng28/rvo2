...
class Circle:
    ...
    def update_visualization(self, viewer):
        if not RVO_RENDER:  # When RVO_RENDER is TRUE, the program will never return
            return
...
def main():
    ...
    while not circle.reached_goal():
        if RVO_RENDER:
            if viewer is None:
                viewer = rendering.Viewer(750, 750)  # (width,height) in pixels
                viewer.set_bounds(-225, 225, -225, 225)  # My guess, is used to move the (0,0) to centre
